...
pdf.r<-0.5
pdf("posterior_two_countries.pdf",width=(w1/rat[1])*sum(rat)*pdf.r,height=39*pdf.r)
...
text(0,const2-0.8,"Posterior distributions USA",font=2)

plot(NULL,ylim=c(const2,0),xlim=c(-1,1),type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",bty="n")
text(0,const2-0.8,"Posterior distributions Inida",font=2)
...
#Text result output
restab1<-precis(mUSA,prob=0.89,depth=2)
restab2<-precis(mUSA,prob=0.97,depth=2)
...
write.table(restab,"results_numeric_USA.txt",sep="\t",row.names = F)


restab1<-precis(mIndia,prob=0.89,depth=2)
restab2<-precis(mIndia,prob=0.97,depth=2)
...
write.table(restab,"results_numeric_India.txt",sep="\t",row.names = F)
